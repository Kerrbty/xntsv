...
set BUILD_NAME=xntsv_win64

cd %SOURCE_PATH%/dll_source
start /wait _build64.bat
cd %SOURCE_PATH%
...
copy %SOURCE_PATH%\build\amd64\memoryaccess64.sys %SOURCE_PATH%\release\%BUILD_NAME%\
xcopy %SOURCE_PATH%\build\db %SOURCE_PATH%\release\%BUILD_NAME%\db /E /I

copy %QT_PATH64%\bin\Qt5Widgets.dll %SOURCE_PATH%\release\%BUILD_NAME%\
copy %QT_PATH64%\bin\Qt5Gui.dll %SOURCE_PATH%\release\%BUILD_NAME%\
...
copy %QT_PATH64%\plugins\platforms\qwindows.dll %SOURCE_PATH%\release\%BUILD_NAME%\platforms\
